...
def test_retrieval_source_hit_uses_source_family_without_chunk_leak(tmp_path: Path) -> None:
    knowledge = [
        {
            "doc_id": "graduation_biochemistry_requirements_chunk_1",
            "label": 0,
            "domain": "graduation",
            "title": "생화학과 졸업요건",
            "body": "졸업 전공 교양 학점 이수 기준 확인 문장입니다.",
            "source_url": "https://biochemistry.cnu.ac.kr",
        ...
        {
            "doc_id": "graduation_biochemistry_requirements_chunk_2",
            "label": 0,
            "domain": "graduation",
            "title": "생화학과 졸업요건",
            "body": "미래설계상담 과목 이수와 전공선택 인정과목 안내 문장입니다.",
            "source_url": "https://biochemistry.cnu.ac.kr",
            "source_id": "graduation_biochemistry_requirements",
            "metadata": {
                "source_id": "graduation_biochemistry_requirements",
    ...
    qa = [
        {
            "user": "생화학과 졸업요건: 졸업 기준 알려줘",
            "model": "근거는 '미래설계상담 과목 이수와 전공선택 인정과목 안내 문장입니다'입니다.",
            "source_doc_id": "graduation_biochemistry_requirements_chunk_2",
            "source_url": "https://biochemistry.cnu.ac.kr",
            "label": 0,
            "validated": True,
        }
    ...
    assert "chunk_" not in qa[0]["user"]
    assert metrics["top3_source_hit_rate"] == 1.0
    assert metrics["graduation_top3_source_hit_rate"] == 1.0
    assert "top3_doc_id_hit_rate" in metrics
...
def test_retrieval_validator_requires_top_level_doc_id_diagnostic(tmp_path: Path) -> None:
    knowledge_path = tmp_path / "knowledge.json"
    qa_path = tmp_path / "qa.json"
    metrics_path = tmp_path / "retrieval_metrics.json"
    _write_json(knowledge_path, [])
    ...
    _write_json(
        metrics_path,
        {
            ...
            "qa_checksum": sha256(qa_path.read_bytes()).hexdigest(),
            "retrieval_strategy": "lexical_metadata_label_hint",
            "metadata_fields": ["source_id", "source_stage", "source_parser_type"],
            "top1_label_accuracy": 1.0,
            "top3_source_hit_rate": 1.0,
            "per_label_failure_counts": {str(label): 0 for label in range(5)},
            "row_count": 0,
            "per_label_row_counts": {str(label): 0 for label in range(5)},
            "per_label_top1_label_accuracy": {str(label): 1.0 for label in range(5)},
            "per_label_top3_source_hit_rate": {str(label): 1.0 for label in range(5)},
            "per_label_top3_doc_id_hit_rate": {str(label): 1.0 for label in range(5)},
            "graduation_top3_source_hit_rate": 1.0,
            "graduation_top3_doc_id_hit_rate": 1.0,
            "graduation_curriculum_pdf_hit_rate": 1.0,
            "retrieved_evidence_alignment_rate": 1.0,
            "expected_evidence_alignment_rate": 1.0,
            "alignment_failures": [],
        },
    )

    with pytest.raises(ValueError, match="top3_doc_id_hit_rate is required"):
        validate_retrieval_metrics(
            metrics_path,
            knowledge_path=knowledge_path,
            qa_path=qa_path,
