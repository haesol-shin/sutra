...
def test_plain_recursive_chunking_preserves_calendar_date_and_event_name() -> None:
    ...
    assert all(chunk.boundary_type == "plain_text" for chunk in chunks)
    assert all(chunk.chunk_confidence is None for chunk in chunks)


def test_plain_recursive_chunking_handles_normalized_html_text() -> None:
    text = (
...
def test_plain_recursive_chunking_preserves_obvious_date_location_meal_and_menu() -> None:
    text = (
        "2026-06-16 2학생회관 중식\n"
        "백반, 된장국, 제육볶음\n"
        "2026-06-16 3학생회관 석식\n"
...
def test_plain_recursive_chunking_preserves_stop_and_departure_times() -> None:
...
def test_plain_recursive_chunking_preserves_obvious_title_date_body_block() -> None:
    text = (
...
def test_plain_recursive_chunking_preserves_heading_with_obvious_requirement_line() -> None:
    text = (
        "컴퓨터융합학부 졸업요건\n"
...
def test_general_prose_uses_plain_recursive_chunks_without_confidence() -> None:
    ...
    assert chunks
    assert all(chunk.chunk_confidence is None for chunk in chunks)
    assert all(chunk.strategy == "recursive_plain" for chunk in chunks)


def test_unstructured_text_falls_back_without_confidence() -> None:
    text = "2026학년도교육과정표전공필수전공선택교양핵심" * 20

    chunks = split_source_text(text, label=0, max_chunk_chars=100, max_chunks=3)

    assert chunks
    assert all(chunk.strategy == "recursive_plain_window" for chunk in chunks)
    assert all(chunk.chunk_confidence is None for chunk in chunks)
