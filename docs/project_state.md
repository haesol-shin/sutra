...
- **Legacy Packages**: `src/nlp_term/` was removed on 2026-06-13. The Sutra router is sklearn-free: `src/sutra/service.py` loads the numpy artifact [examples/cnu-campus/model/router_classifier.npz](../examples/cnu-campus/model/router_classifier.npz) and predicts with a pure `char_wb` TF-IDF + linear argmax implementation. It is parity-proven 100% against the frozen `model/classifier.joblib` (evidence: [docs/evidence/router-sklearn-numpy-parity-2026-06-13.json](evidence/router-sklearn-numpy-parity-2026-06-13.json)).
...
The active verified corpus consists of **116 clean documents** merged under [knowledge-index.jsonl](../examples/cnu-campus/data/processed/knowledge-index.jsonl):
...
4. **Service Modes**: `service.ask()` supports `mode="default"`, `mode="tool_only"`, and `mode="router"` (Task1-classifier forced tool routing for dining/notices, RAG for calendar/graduation/shuttle; Option A — forced tools fire even on empty RAG). **Grading path = router** (decided 2026-06-12, commit 3aa6d41): `sutra batch` (chatbot.sh chat + realtime) now calls `mode="router"`. The numpy router artifact resolves via `SUTRA_ROUTER_ARTIFACT_PATH` then `<workspace_root>/model/router_classifier.npz`; a missing or unloadable artifact falls back to RAG. Output schema unchanged (cls=question/label, chat/realtime=user/model).
5. **Submission bootstrap (merged, 2026-06-12)**: the engine no longer uses `joblib` or scikit-learn; router mode uses the numpy `.npz` router artifact through `src/sutra/service.py`. The grading `submission` branch is frozen and unaffected.
