...
- **Legacy Packages**: The package [src/nlp_term](../src/nlp_term) is legacy Task 1 classifier source context only. Sutra runtime does not import `nlp_term`; router mode loads the trained `classifier.joblib` inline via `src/sutra/service.py::_load_classifier`.
