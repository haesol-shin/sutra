...
Type: decision
Decision: Convert the submission package into a minimal git-clone bootstrap. The zip ships only 6 files (chatbot.sh, src/classifier.ipynb, data/test_{cls,chat,realtime}.json, model/classifier.joblib); all source code, the CNU corpus, and prompts are cloned fresh at run time from the public repo's `submission` branch.
Reason: The deadline (2026-06-12) arrived with RAG/tool/UI work still in flight; cloning a live branch lets those be edited after submission without touching the frozen zip. git clone (not pip install) is used because pip would only install the `sutra` package and would miss nlp_term, the workspace corpus, prompts, and classifier.joblib.
Consequence: Grader runs classifier.ipynb (Task 1, self-contained: bundled joblib + inline predict, scikit-learn pinned to 1.7.2, no nlp_term, no clone) then `bash chatbot.sh` (no args) which clones `submission` to /content local disk, installs with uv, downloads the GGUF, serves, and (30s-timeout / non-tty default) generates the Task 2/3 files or launches the UI behind cloudflared. The router classifier is wired via absolute SUTRA_CLASSIFIER_PATH to the bundled joblib (the clone's model/ is gitignored). The graded code is therefore the `submission` HEAD at grade time, not the zip — `submission` must be treated as frozen after grading and only fast-forwarded with verified commits. Verified: local rehearsal + real public @submission clone both produce chat/realtime with 0 fallback and live notice/dining routing; full Colab install path (uv --system, GGUF download, GPU server) still to be rehearsed by the user.
Links: `chatbot.sh`, `src/classifier.ipynb`, `scripts/build_submission.py`, `src/sutra/service.py` (nlp_term-free router), `pyproject.toml` (commits eb8c39a, 29a738c, 8e402c0)
